...
def save_blog_text(html_content: str, save_dir: str) -> None:
    ...
    #if len(text) == 0:
    text = '\n'.join(div.get_text() for div in raw_text.find_all('div', recursive=False))
    if len(text) != 0:
        if len(raw_text.find_all('div', recursive=False)) == 1:
            for br in raw_text.find_all('div', recursive=False)[0].find_all('br'):
                br.replace_with('\n')
